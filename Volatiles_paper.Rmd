...
patho.mse <- ddply(patho.m, c("Strain","Time") , summarise, N = length(Growth), mean = mean(Growth), sd   = sd(Growth), se   = sd / sqrt(N))
patho.mse

basidio.plot <- ggplot(patho.mse, aes(x=Time, y=mean, group=Strain, color=Strain)) + geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.1) +geom_line() + geom_point()+ theme(legend.position = "none")+ ggtitle("Basidiobolus") +ylab("")
...
alternaria.plot <- ggplot(patho.mse, aes(x=Time, y=mean, group=Strain, color=Strain)) + geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.1) +geom_line() + geom_point()+ theme(legend.position = "none")+ ggtitle("Alternaria")+ ylab("Percent Petri Dish")
...
rhizo.plot <- ggplot(patho.mse, aes(x=Time, y=mean, group=Strain, color=Strain)) +  geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.1) + geom_line() + geom_point()+ theme(legend.position = "none") + ggtitle("Rsol") + theme(legend.position = "bottom") + ylab("")

#Ncrassa
patho <- Ncra
patho.m <- melt(as.data.frame(patho), id.vars = "Bacteria")
colnames(patho.m) <- c("Strain","Time","Growth")
ncrassa.plot <- ggplot(patho.mse, aes(x=Time, y=mean, group=Strain, color=Strain)) + 
    geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.1) +
    geom_line() + geom_point()+ ggtitle("Neurospora") +theme(legend.position = "none")+ylab("")
ncrassa.plot

leg.plot <- ggplot(patho.mse, aes(x=Time, y=mean, group=Strain, color=Strain)) + 
    geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.1) +
    geom_line() + geom_point()+ ggtitle("Neurospora") +theme(legend.position = "none")+ylab("")
...
library(cowplot)
plot_grid(alternaria.plot, basidio.plot, ncrassa.plot, rhizo.plot, leg, labels = c('A', 'B', 'C', 'D', 'E'), label_size = 12)
...
```{r tempname, echo=FALSE, out.width = "90%", fig.pos = "H", fig.cap="Table of differentially produced volatiles"}
...
library(cowplot)
plot_grid(micro_con, micro_dmpz, micro_und, micro_an, micro_dmts, labels = c('A', 'B', 'C', 'D', 'E'), label_size = 12)
title = cowplot::ggdraw() + cowplot::draw_label("40X "~italic(Rhizopus)~ " Hyphae Exposed to Volatiles", size = 20)
top_row = cowplot::plot_grid(micro_con, micro_dmpz, micro_und, labels = c('A', 'B', 'C'), label_size = 12, ncol=3)
bottom_row = cowplot::plot_grid(micro_an, micro_dmts, labels = c('D', 'E'),label_size = 12, ncol=2)

cowplot::plot_grid(title, top_row, bottom_row, ncol=1)
```
