...
Basidi<- read_excel("Volatile.xlsx", sheet = "Basidi", range="A1:D10")
Alt<- read_excel("Volatile.xlsx", sheet = "Alt", range="A1:D10")
Rhizo<- read_excel("Volatile.xlsx", sheet = "Rhizo", range="A1:D12")
Ncra<- read_excel("Volatile.xlsx", sheet = "Ncrassa_donut", range="A1:D10")
...
library(ddply)
patho.mse <- summarySE(patho.m, measurevar="Growth", groupvars=c("Strain", "Time"))
...
basidio.plot <- ggplot(dfm, aes(x=Time, y=Growth, group=Strain, color=Strain)) + 
    ...
    geom_line() + geom_point()+ theme(legend.position = "none")+ ggtitle("Basidiobolus") +ylab("") + stat_compare_means(method = "anova", label.y = 60) + stat_compare_means(aes(label = ..p.signif..), method = "t.test", ref.group = "None")
...
alternaria.plot <- ggplot(dfm, aes(x=Time, y=Growth, group=Strain, color=Strain)) + 
    geom_errorbar(aes(ymin=Growth-sd, ymax=Growth+sd), width=.1) +
    geom_line() + geom_point()+ theme(legend.position = "none")+ ggtitle("Alternaria")+ ylab("Percent Petri Dish")+ stat_compare_means(method = "anova", label.y = 35) + stat_compare_means(aes(label = ..p.signif..), method = "t.test", ref.group = "None")
...
rhizo.plot <- ggplot(dfm, aes(x=Time, y=Growth, group=Strain, color=Strain)) + 
    geom_errorbar(aes(ymin=Growth-sd, ymax=Growth+sd), width=.1) +
    geom_line() + geom_point() +ggtitle("Rsol") + theme(legend.position = "bottom") + ylab("")+ stat_compare_means(method = "anova", label.y = 60) + stat_compare_means(aes(label = ..p.signif..), method = "t.test", ref.group = "None")
rhizo.plot
...
ncrassa.plot <- ggplot(dfm, aes(x=Time, y=Growth, group=Strain, color=Strain)) + 
    geom_errorbar(aes(ymin=Growth-sd, ymax=Growth+sd), width=.1) +
    geom_line() + geom_point()+ ggtitle("Neurospora") +theme(legend.position = "none")+ylab("")+ stat_compare_means(method = "anova", label.y = 60) + stat_compare_means(aes(label = ..p.signif..), method = "t.test", ref.group = "None")

leg.plot <- ggplot(dfm, aes(x=Time, y=Growth, group=Strain, color=Strain)) + 
    geom_errorbar(aes(ymin=Growth-sd, ymax=Growth+sd), width=.1) +
    geom_line() + geom_point()+ ggtitle("Neurospora")
leg.plot
...
grid.newpage()
grid.arrange(alternaria.plot, basidio.plot, ncrassa.plot, rhizo.plot, leg, nrow = 2, ncol=3,layout_matrix = rbind(c(1, 2, 4), c(3, 5, 4)))
...
grid.newpage()
Venn <- draw.triple.venn(area1 = 34, area2 = 31, area3 = 29, n12 = 16, n23 = 23, n13 = 16, n123 = 13, category = c("Bsub", "Smar", "Spro"), lty = rep("solid", 3), fill = c("lightcoral", "olivedrab3", "lightblue2"))
...
```{r meta_anova, echo=FALSE, out.width = "90%", fig.pos="H", fig.cap="ANOVA results showing 95% confidence or higher 11 compounds differentially produced by Bacteria"}
knitr::include_graphics("aov_2_dpi600.png")
```


```{r table2, echo=FALSE, message=FALSE, out.width = "100%", warning=FALSE, fig.pos = "H", fig.cap="Table of differentially produced volatiles"}
...
```{r include=FALSE}
straw <- read_excel("Volatile.xlsx", sheet = "Strawberry", range="A1:B13")

straw.plot <- ggplot(straw, aes(x=State, y=Spores, fill=State)) + geom_boxplot() + stat_compare_means(method = "t.test", label.x="With Smar") + xlab("Condition") + theme(legend.position = "none")

```

```{r strawberry, out.width="90%", fig.cap=Spores recovered from strawberries innoculated with Rstol in the presence of S. marcescens}
