...
timelap <- read_excel("./Data_in/Volatile.xlsx", sheet = "Serratia Multistrain Analysis")
...
final.data
pd <- position_dodge(1)
timelap.bar <- ggplot(final.data, aes(x=Time, y=mean, fill=Strain)) + geom_bar(stat="identity", position=pd) + geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.1, position=pd) + xlab("Bacterial Incubation Time (Hours) Before Rstol Innoculation") + ylab("Mycelial growth (cm^2) of Rstol") + theme_classic() + ggtitle("48 Hours of Bacterial Growth is Sufficent to Inhibit Rstol Mycelial Growth")
...
png("R-plots/time_assay.png", width = 800)
timelap.bar
dev.off
...
```{r include=FALSE}
straw <- read_excel("./Data_in/Volatile.xlsx", sheet = "Strawberry", range="A1:B13")

straw.plot <- ggplot(straw, aes(x=State, y=Spores, fill=State)) + geom_boxplot() + stat_compare_means(method = "t.test", label.x="With Smar") + xlab("Condition") + theme(legend.position = "none") + ggtitle("Smar Reduces Spore Production in Rstol")
...
Mucor<- read_excel("./Data_in/Volatile.xlsx", sheet = "Mucor")
AF<- read_excel("./Data_in/Volatile.xlsx", sheet = "AF")
Rhizo<- read_excel("./Data_in/Volatile.xlsx", sheet = "Rhizo")
Ncra<- read_excel("./Data_in/Volatile.xlsx", sheet = "Ncrassa")
...
Mucor.plot <- ggplot(Mucor.mse, aes(x=Time, y=mean, group=Strain, color=Strain)) + geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.1) +geom_line() + geom_point()+ theme(legend.position = "none")+ ggtitle(expression(italic("Mucor"))) +ylab("")+ xlab("")
...
AF.plot <- ggplot(AF.mse, aes(x=Time, y=mean, group=Strain, color=Strain)) + geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.1) +geom_line() + geom_point()+ theme(legend.position = "none")+ ggtitle(expression(italic("Aspergillus")))+ ylab("Percent Petri Dish")
...
rhizo.plot <- ggplot(Rhizo.mse, aes(x=Time, y=mean, group=Strain, color=Strain)) +  geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.1) + geom_line() + geom_point()+ theme(legend.position = "none") + ggtitle("Rstol") + theme(legend.position = "none") + ylab("") + xlab("")
...
ncrassa.plot <- ggplot(Ncra.mse, aes(x=Time, y=mean, group=Strain, color=Strain)) + 
    geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.1) +
    geom_line() + geom_point()+ ggtitle(expression(italic("Neurospora"))) +theme(legend.position = "none")+ylab("")+ xlab("")


leg.plot <- ggplot(Ncra.mse, aes(x=Time, y=mean, group=Strain, color=Strain)) + 
    geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.1) +
    geom_line() + geom_point()+ ggtitle("Neurospora") +theme(legend.position = "bottom")+ylab("")
...
library(cowplot)
fungi_grid <- plot_grid(AF.plot, Mucor.plot, ncrassa.plot, rhizo.plot, labels = c('A', 'B', 'C', 'D'), ncol = 2)
plot_grid(fungi_grid, leg, ncol = 1, rel_heights = c(1, .1))
png("./R-plots/OtherFungi.png")
plot_grid(fungi_grid, leg, ncol = 1, rel_heights = c(1, .1))
dev.off()
```
...
Samples <- read_excel("./Data_in/GC_data.xlsx", sheet = "Total_production")

Samples.df <- as.data.frame(Samples)

rownames(Samples.df) <- Samples$`Sample`

vol_total <- ggplot(Samples.df, aes(x=Samples.df$`Sample`, y=Samples.df$`Detected Volatiles (ng)`, fill=Samples.df$`Detected Volatiles (ng)`)) + geom_bar(stat="identity") + guides(fill=FALSE) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + ylab("ng of Volatiles") + xlab("Sample") + scale_fill_gradient(low="red", high="blue") + geom_hline(yintercept=200, linetype="dotted")
...
volatable <- read_excel("./Data_in/GC_data.xlsx", sheet = "Volatable")
...
volatable.table <- volatable.sub %>%
  kable(col.names = c("Compound", "Sample", "Mass Detected (ng)")) %>%
  kable_styling(bootstrap_options = c("striped", "hover"))%>%
  save_kable("./R-plots/volatable.pdf")

help(kable)
knitr::kable(volatable.sub, col.names = c("Compound", "Sample", "Mass Detected (ng)"))
kable_styling(volatable.sub, bootstrap_options = c("striped", "hover"))
save_kable("./R-plots/volatable.pdf")
...
grid.newpage()
  Venn <- draw.triple.venn(area1 = 27, area2 = 25, area3 = 23, n12 = 4, n23 = 16, n13 = 4, n123 = 4, category = c("Bsub", "Smar", "Spro"), lty = rep("solid", 3), fill = c("lightcoral", "olivedrab3", "lightblue2"))

  
png("./R-plots/Venn.png")
...
#NMDS Plot
library(vegan)
volatiles <- read_excel("./Data_in/GC_data.xlsx", sheet = "NMDS_1")
volatiles_names <- read_excel("./Data_in/GC_data.xlsx", sheet = "NMDS_2")
...
ordihull(vol.rel_NMS,groups=treat,draw="polygon",col=c("green", "red", "blue"),label=F, scaling = 3, main = expression(air == 2), cex = 1)
legend(x="bottomleft", legend=unique(treat), col=c("green", "red", "blue"), pch=16)

png("./R-plots/NMDS.png")
treat=c(rep("Bsub",6),rep("Smar",6),rep("Spro",8))
ordiplot(vol.rel_NMS,type="points")
ordihull(vol.rel_NMS,groups=treat,draw="polygon",col=c("green", "red", "blue"),label=F, scaling = 3, main = expression(air == 2), cex = 1)
legend(x="bottomleft", legend=unique(treat), col=c("green", "red", "blue"), pch=16)
...
mSet<-PlotSubHeatMap(mSet, "heatmap_3_", "png", 600, width=NA, "norm", "row", "correlation", "average","bwm", "tanova", 63, "overview", T, T, T, F)
...
anov_res <- read_excel("./Data_in/GC_data.xlsx", sheet = "ANOVA")
...
abund <- read_excel("./Data_in/Volatile.xlsx", sheet = "Abundant_VOC_RS_24")
...
abund.plot = ggplot(abund.m, aes(x=Compound, y=abund.m$value, fill=Compound)) + geom_boxplot() + ylab("Diameter of Rstol mycelium (cm)") + xlab("10mg of Compound") + coord_flip() +  guides(fill=FALSE)
...
genus_gc <- read_excel("/Volumes/GoogleDrive/My Drive/Lab/Serratia/Volatiles_Rmd/GC_data.xlsx", sheet = "Comparative_genus")
genus_gc
